...
def _is_tie_point(current_props: Dict[str, Any], neighbor_props: Dict[str, Any]) -> bool:
    """Same Section, but a different cable (Distribution or Letter or Conduit Type differs)."""
...
def dfs_ordered_vaults(
    ...
    """
    Traversal implementing the field rules:
    ...
    def _is_tie_neighbor(base_fid: int, neighbor_fid: int) -> bool:
        pa = feature_props[base_fid]
        pb = feature_props[neighbor_fid]
    ...
    def _walk_feature(fid: int, entry_node: Tuple[int, int], v_state: Dict[int, int]):
        ...
        # neighbors at node nk including snapped mid-segment tees within NODE_SNAP_FT
        def _neighbors_at_node(nk: Tuple[int, int]) -> List[int]:
            xy = _nk_to_xy(nk)
            # true shared-vertex neighbors
            shared = {
                nf
                for nf in node_to_features.get(nk, set())
                if nf != fid and nf not in visited_fids and _is_tie_neighbor(fid, nf)
            }
            # snapped tees in SAME Section (not same cable)
        ...
        for nk in nodes:
            ...
            # 2) split neighbors by Distribution priority
            ...
            # 3) detour LOWER (smallest fiber count first)
            for nf in sorted(
                lowers,
            ...
            # 4) if CURRENT is 24, detour equal-24 now (B → A → others)
            for nf in sorted(
                equal24,
    ...
    for sec in sections_to_run:
        ...
        # Prefer T3-anchored start (highest Distribution Type; tie → A first)
